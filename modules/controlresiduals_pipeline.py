...
class MultiControlNetResidualsPipeline:
    def __init__(self, hf_controlnet_names, cond_scale):
        ...
        for controlnet_name in hf_controlnet_names:
            self.controlnets.append(
                ControlNetModel.from_pretrained(
                    controlnet_name, torch_dtype=torch.float16
                )
            )
        ...
        self.mlsd_processor = MLSDdetector.from_pretrained(
            "lllyasviel/Annotators",
            cache_dir=cache_dir,
        )
        self.openpose_processor = OpenposeDetector.from_pretrained(
            "lllyasviel/Annotators",
            cache_dir=cache_dir,
        )
        self.hed_processor = HEDdetector.from_pretrained(
            "lllyasviel/Annotators",
            cache_dir=cache_dir,
        )
        self.lineart_anime_processor = LineartAnimeDetector.from_pretrained(
            "lllyasviel/Annotators", cache_dir=cache_dir
        )
        self.lineart_processor = LineartDetector.from_pretrained(
            "lllyasviel/Annotators", cache_dir=cache_dir
        )
        self.normalbae_processor = NormalBaeDetector.from_pretrained(
            "lllyasviel/Annotators", cache_dir=cache_dir
        )
    ...
    def prepare_controlnet_input_image(self, controlnet_model, image):
        if "canny" in controlnet_model:
            ...
            detect_resolution = min(h, w)
            image_resolution = min(h, w)
            o_image = self.lineart_anime_processor(
                image,
            ...
            detect_resolution = min(h, w)
            image_resolution = min(h, w)
            o_image = self.lineart_processor(
                image,
                detect_resolution=detect_resolution,
                image_resolution=image_resolution,
            )
            ...
            raise Exception(
                f"ControlNet model {controlnet_model} is not supported at this time."
            )
    ...
    def prep_control_images(
        ...
        for ctrl_name in self.controlnet_names:
            ...
            for i, image in enumerate(images):
                ...
                if save_outputs:
                    prep_image.save(
                        os.path.join(out_dir, "{}_{:04d}.png".format(epoch, i))
                    )
                prep_image = self.prepare_images(
                    [prep_image], width, height, 1, 1, "cuda", self.multicontrolnet
                )
    ...
    def __call__(
        self, control_model_input, t, controlnet_prompt_embeds, guess_mode=False
    ):
        control_model_input = rearrange(control_model_input, "b c f h w -> (b f) c h w")

        controlnet_prompt_embeds = torch.cat(
            [controlnet_prompt_embeds] * (len(self.prep_images[0]) // 2)
        )
