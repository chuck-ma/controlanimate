...
def video_to_high_fps(
    output_name,
    video_file_path,
    audio_path,
    processed_file_save_dir,
    time_interval,
    fps_ffmpeg,
    crf=17,
    ...
    """
    The purpose of this function is to increase the framerate of the input video (e.g., generated from output frames) by interpolation.
    It also adds audio to the input video if the audio file exists.
    params:
...
class FFMPEGProcessor:
    def __init__(self, cmd, std_in=False, std_out=False):
        self.process = Popen(
            cmd,
...
if __name__ == "__main__":
    ...
    time_interval = f"-ss {start_time}" + f" -to {end_time}" if len(end_time) else ""

    final_process = video_to_high_fps(
        "test.mp4",  # Name of the final output
